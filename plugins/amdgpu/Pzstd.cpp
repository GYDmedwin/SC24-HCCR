...
static std::uint64_t handleOneInput(const Options &options,
                             ...
                             FILE* outputFd,
                             SharedState& state) {
  auto inputSize = options.input_len;
  ...
  std::uint64_t bytesWritten;
  {
    ...
    if (!options.decompress) {
      ...
      readExecutor.add(
          [&state, &outs, &executor, input_buffer, inputSize, &options, &bytesRead] {
      ...
      //TODO:这里需要阻塞执行
      write_wait(options.id, options.data_type);
      // Start writing
      bytesWritten = writeFile(state, outs, input_buffer, outputFd, options.decompress, options.isLast);
      write_notify(options.data_type);
    } 
    // else {
...
int pzstdMain(const Options &options) {
  ...
  auto input_buffer = options.inputBuffer;
  if (input_buffer == nullptr) {
    return -1;
  }

  // Get the output file
  auto outputFd = options.outputFile;
  if (outputFd == nullptr) {
    return -1;
  }
  // auto closeOutputGuard = makeScopeGuard([&] { std::fclose(outputFd); });
  // (de)compress the file
  handleOneInput(options, input_buffer, outputFd, state);
  if (state.errorHolder.hasError()) {
      return -1;
  }
...
static void compress(
  ...
  setLastCompressTime(compress_end);
}
...
static size_t calculateStep(
  ...
  return size_t{1} << (params.cParams.windowLog);
...
FileStatus bufferStatus(std::uint64_t *totalBytesRead, size_t total_size) {
 ...
 */
static FileStatus
readData(BufferWorkQueue& queue, size_t chunkSize, size_t size, void* input_buffer,
        size_t total_size, std::uint64_t *totalBytesRead) {
  Buffer buffer(size);
  size_t bytesRead = 0;
  while (!buffer.empty()) {
    bytesRead = std::min(chunkSize, buffer.size());
    if(bytesRead + *totalBytesRead > total_size){
      bytesRead = total_size - *totalBytesRead;
    }
    //TODO: 可能直接相加，容易出问题。
    // memcpy(buffer.data(), input_buffer + once_read, bytesRead);
    memcpy(buffer.data(), input_buffer + *totalBytesRead, bytesRead);
    *totalBytesRead += bytesRead;
    queue.push(buffer.splitAt(bytesRead));
    auto status = bufferStatus(totalBytesRead, total_size);
...
std::uint64_t asyncCompressChunks(
  ...
  while (status == FileStatus::Continue && !state.errorHolder.hasError()) {
    ...
    status = readData(*in, ZSTD_CStreamInSize()*4, step, input_buffer, size, &bytesRead);
  }
...
std::uint64_t writeFile(
  ...
  if(input_buffer != nullptr && isLast != nullptr){
    free(isLast);
  }
  return bytesWritten;
}
