...
int write_fp(FILE *fp, const void *buf, const size_t buf_len)
{
	...
	len_write = fwrite(buf, 1, buf_len, fp);  //此处fwrite方法即为Linux提供的底层方法
...
FILE *open_img_file(char *path, bool write, size_t *size)
{
	...
	if (opts.stream)  //opts->options 许多可选操作，是个结构体
	...
	if (write)
		ret = write_fp(fp, size, sizeof(*size)); //此处是将文件的大小，即前八个字节先行写入
	else
...
int write_img_file(char *path, const void *buf, const size_t buf_len)
{
	...
	fp = open_img_file(path, true, &len); //打开文件，获取各种流后获得fp，同时先将文件大小写入。
	if (!fp)
		return -errno;
...
int sdma_copy_big_bo(struct kfd_criu_bo_bucket *bo_buckets, void *userptr, int i, amdgpu_device_handle h_dev,
		 uint64_t max_copy_size, enum sdma_op_type type, threadpool pool, FILE *bo_contents_fp, int* num_bos)
{
	uint64_t size, gpu_addr_src, gpu_addr_dest, gpu_addr_ib;
	uint64_t gpu_addr_src_orig, gpu_addr_dest_orig;
	amdgpu_va_handle h_va_src, h_va_dest, h_va_ib;
	...
	err = alloc_and_map(h_dev, 28, AMDGPU_GEM_DOMAIN_GTT, &h_bo_ib, &h_va_ib, &gpu_addr_ib,
			           (void **)&ib);
	if (err) {
		pr_perror("failed to allocate and map ib/rb");
		goto err_ib_gpu_alloc;
	...
	size_t once_combine_size;
	size_t CO_size = 16*1024*1024;
	while(bytes_remain > 0){
		/* prepare ring buffer/indirect buffer for command submission
		* each copy packet is 7 dwords so we need to alloc 28x size for ib
		*/
		once_combine_size =0;
		while(once_combine_size < CO_size){
			memset(&cs_req, 0, sizeof(cs_req));
			memset(&fence, 0, sizeof(fence));
			memset(ib, 0, 28);
			...
			*/
			...
			once_combine_size += copy_size;
			
			if(copy_size == bytes_remain)
				break;
		}
	
		struct Options_Args option_args;
		option_args.decompress = false;
		option_args.compressionLevel = 3;
		option_args.numThreads = 8;
		option_args.maxWindowLog = 23;
		option_args.overwrite = false;
		option_args.keepSource = true;
		option_args.checksum = false;
		option_args.verbosity = 1;
		option_args.writeMode = Regular;
		option_args.id = *num_bos;
		option_args.data_type = 1;
		option_args.input_buffer = userptr + total_copy_size;
		option_args.input_len = once_combine_size;
		option_args.outputFile = bo_contents_fp;

		if(bytes_remain <= 0)
			option_args.isLast = userptr;
		else{
			option_args.isLast = NULL;
			(*num_bos)++;
		}
			
		OptionRef ref = getOption(option_args);
		
		thpool_add_work(pool, pzstdMain_C, (void*)ref);

		total_copy_size += once_combine_size;
	}
	gpu_addr_src = gpu_addr_src_orig;
	gpu_addr_dest = gpu_addr_dest_orig;
...
err_dest_va:
	...
	if (err)
		pr_perror("dest bo free failed");
err_bo_list:
		free_and_unmap(28, h_bo_ib, h_va_ib, gpu_addr_ib, ib);
err_dest_bo_prep:
	err = amdgpu_bo_va_op(h_bo_src, 0, size, gpu_addr_src, 0, AMDGPU_VA_OP_UNMAP);
	if (err)
		pr_perror("failed to GPU unmap the src BO %lx, size = %lx", gpu_addr_src, size);
...
void *dump_bo_contents(void *_thread_data)
{
	...
	thpool = thpool_init(8); //此处控制整个项目CRIU侧线程池的大小
	getGPUpool(thpool);
	...
	for (i = 0; i < thread_data->num_of_bos; i++) {
		...
		if (bo_buckets[i].size > to_big){
			/* perform sDMA based vram copy (Big BO)*/
			ret = sdma_copy_big_bo(bo_buckets, buffer, i, h_dev, max_copy_size, SDMA_OP_VRAM_READ, thpool, bo_contents_fp, &num_bos);
			if (ret) {
			...
			uint64_t once_total_byte = 0;
			do{
				// 下方代码均是为了将过小的BO合并而写的
				/* perform sDMA based vram copy (Small BO)*/
				ret = sdma_copy_bo(bo_buckets, buffer+once_total_byte, i, h_dev, max_copy_size, SDMA_OP_VRAM_READ);
				if (ret) {
					pr_err("Failed to drain the BO using sDMA: bo_buckets[%d]\n", i);
					break;
				}
				plugin_log_msg("** Successfully drained the BO using sDMA: bo_buckets[%d] **\n", i);
				once_total_byte += bo_info[i]->size;
				i++;
				while(i<thread_data->num_of_bos){
					if (bo_buckets[i].gpu_id != thread_data->gpu_id){
						i++;
						continue;
					}
				...
				if(i == thread_data->num_of_bos) break;
				if(once_total_byte + bo_info[i]->size >= to_big || bo_info[i]->size > to_big){
					i--;
					break;
				}
			}while(once_total_byte < to_big);
			...
			option_args.writeMode = Regular;
			option_args.id = num_bos;
			option_args.input_buffer = buffer;
			option_args.input_len = once_total_byte;
			option_args.outputFile = bo_contents_fp;
			option_args.isLast = buffer;
			option_args.data_type = 1;
			OptionRef ref = getOption(option_args);

			thpool_add_work(thpool, pzstdMain_C, (void*)ref);
		}
	}
...
int amdgpu_plugin_dump_file(int fd, int id)
{
	...
	struct stat st, st_kfd;    //stat结构体为内核中的，可以上网搜其内部有什么字段
	unsigned char *buf;
	CriuKfd *e = NULL;
	int ret = 0;
	size_t len;

	if (fstat(fd, &st) == -1) {//fstat用来将fd所指向的文件状态复制到st中，因而目前即将转储的文件的状态均保存在st中
		pr_perror("amdgpu_plugin: fstat error");
		return -1;
	}

	ret = stat(AMDGPU_KFD_DEVICE, &st_kfd); //此处为检测/dev/kfd是否正确打开，并在st_kfd中保存其状态
	if (ret == -1) {
	...
	*   字符设备驱动major号 & minor号:
	*   1. major号用来标识与设备相关联的驱动程序。许多设备使用相同的major号。
	*   2. 设备驱动程序使用minor号<minor>来辨别单个物理或逻辑设备
	*/
	if (major(st.st_rdev) != major(st_kfd.st_rdev) || minor(st.st_rdev) != 0) {  //__dev_t st_rdev;		/* Device number, if device.  */
		...
		criu_render_node__pack(&rd, buf); //该方法是自动生成的，意为将rd序列化后传至buf

		snprintf(img_path, sizeof(img_path), IMG_RENDERD_FILE, id);
		ret = write_img_file(img_path, buf, len); //将buf上的内容写入镜像文件
		if (ret) {
			xfree(buf);
			return ret;
		}
	...
	pr_info("amdgpu_plugin: devices:%d bos:%d objects:%d priv_data:%lld\n", args.num_devices, args.num_bos,
		args.num_objects, args.priv_data_size);  //num_objects = num_queues + num_events + num_svm_ranges
