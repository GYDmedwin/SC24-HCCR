...
#include "plugins/amdgpu/Options_CWrapper.h"
#include "plugins/amdgpu/Pzstd_CWrapper.h"
#include "thpool.h"
...
void syncThpool(void){
	thpool_wait(pool);
}
void destoryThpool(void){
	thpool_destroy(pool);
}
...
/* page-server xfer */
static int write_pages_to_server(struct page_xfer *xfer, int p, int q, unsigned long len)
{
...
/* local xfer */
static int write_pages_loc(struct page_xfer *xfer, int pipe_read, int pipe_write, unsigned long len)
{
	void* buffer = malloc(len);
	size_t read_size = read(pipe_read, buffer, len);
	struct Options_Args option_args;
	OptionRef ref;

	option_args.decompress = false;
	option_args.compressionLevel = 3;
	option_args.numThreads = 2;
	...
	option_args.input_len = len;
	option_args.outputFile = fdopen(img_raw_fd(xfer->pi), "a");
	option_args.isLast = buffer;
	option_args.data_type = 0;
	ref = getOption(option_args);
...
int page_xfer_predump_pages(int pid, struct page_xfer *xfer, struct page_pipe *pp)
{
	...
	list_for_each_entry(ppb, &pp->bufs, l) {
		...
		for (i = 0; i < aux_len; i++) {
			...
			if (xfer->write_pages(xfer, ppb->p[0], ppb->p[1], iov.iov_len)) {
				munmap(userbuf, BUFFER_SIZE);
				return -1;
			}
		}
...
int page_xfer_dump_pages(struct page_xfer *xfer, struct page_pipe *pp)
{
	...
	list_for_each_entry(ppb, &pp->bufs, l) {
		...
		for (i = 0; i < ppb->nr_segs; i++) {
			...
			if (xfer->write_pagemap(xfer, &iov, flags))
				return -1;
			if ((flags & PE_PRESENT) && xfer->write_pages(xfer, ppb->p[0], ppb->p[1], iov.iov_len))
				return -1;
		}
	}
...
static int page_server_add(int sk, struct page_server_iov *pi, u32 flags)
{
	...
	while (len > 0) {
		...
		if (lxfer->write_pages(lxfer, cxfer.p[0], cxfer.p[1], chunk))
			return -1;
